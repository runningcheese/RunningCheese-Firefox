...
//老板键
//默认为 Ctrl + ~，如需要修改，可在 Firefox\protable.ini 倒数第二行中修改。
...
keys['F10'] = function(){var newtabs=["https://weibo.com/","https://www.inoreader.com/","https://www.twitter.com/","https://www.runningcheese.com/"];var i=0;while(i<=newtabs.length-1){gBrowser.selectedTab=gBrowser.addTrustedTab(newtabs[i]);i=i+1;}};//一键打开标签组
...
keys['Alt+E'] = 'var url = "about:reader?url=" + gBrowser.currentURI.spec; gBrowser.addTrustedTab(url, {triggeringPrincipal: gBrowser.contentPrincipal});';//阅读模式
...
keys['Alt+V'] =function() {gBrowser.selectedTab = gBrowser.addTrustedTab(readFromClipboard());}; //打开剪切板地址
...
keys["Ctrl+Alt+F"]  = function() {FileUtils.getFile('UChrm',['Local', 'Firemin','Firemin.exe']).launch();};//启动内存管理软件
